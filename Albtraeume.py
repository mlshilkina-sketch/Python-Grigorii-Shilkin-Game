...
def get_integer_input(prompt):
    while True:
        try:
            ...
            print("Fehler: Dies ist keine ganze Zahl. Bitte versuchen Sie es erneut.")
...
def start(x, y, spieler_hp, hunger, angst_punkte, invetar, gold, punkte, ziel):
   
    hunger = get_integer_input("\nWie viel Hunger willst du haben? (Nicht weniger als 40 und nicht mehr als 110) ")
    hunger = hunger //  10 * 10
    if hunger >= 110:
        hunger = 110
    if hunger <= 40:
    ...
    ziel_x = get_integer_input("\nWelches x-Wert willst du für das Ziel haben? (nicht größer als 100 und nicht weniger -100) ")
    ziel_x = ziel_x //  10 * 10
    if ziel_x <= -100:
        ziel_x = -100
    if ziel_x >= 100:
    ...
    ziel_y = get_integer_input("\nWelches y-Wert willst du für das Ziel haben? (nicht größer als 100 und nicht weniger -100) ")
    ziel_y = ziel_y //  10 * 10
    if ziel_y >= 100:
        ziel_y = 100
    if ziel_y <= -100:
    ...
    x = get_integer_input("\nWelches x-Wert willst du für den Start haben? (nicht größer als 100 und nicht weniger -100) ")
    x = x //  10 * 10
    if x <= -100:
        x = -100
    if x >= 100:
    ...
    y = get_integer_input("\nWelches y-Wert willst du für den Start haben? (nicht größer als 100 und nicht weniger -100) ")
    y = y //  10 * 10
    if y >= 100:
        y = 100
    if y <= -100:
        y = -100
    gold = get_integer_input("\nWie viel Gold willst du haben? (nicht mehr als 50) ")
    if gold >= 50:
        gold = 50
    if gold <= 0:
        gold = 0
    angst_punkte = 0
    return(x, y, spieler_hp, hunger, angst_punkte, invetar, gold, punkte, ziel)
...
def schatztruhe(spieler_hp, gold, invetar, angst_punkte):
    ...
    logger.info("Du findest einen schwarz gekleideten Zauberer.")
    if (wahl).lower() == "ja":
        ...
        if inhalt =="gold":
            ...
            doppelt_print("Du findest ein Schwert!")
...
def monster_kampf(invetar, punkte, gold, schaden, angst_punkte, monster_name, monster_hp, monster_starke):
    ...
    logger.info(f"{monster_name} greift an.")
    global spieler_hp
    print(f"Deine HP: {spieler_hp} | {monster_name} HP: {monster_hp}")
    if "schwert" in invetar:
        aktion2 = input("Willst du dein Schwert benutzen? (Ja/Nein) ")
        if (aktion2).lower() == "ja":
            if "schwert" in invetar:
    ...
    while spieler_hp > 0 and monster_hp > 0:
        ...
        if (aktion).lower() == "angreifen" or aktion == "1":
        ...
        elif (aktion).lower() == "heilen" or aktion == "2":
    ...
    if spieler_hp > 0:
        ...
        if gewinn == "Gold":
            ...
            spieler_hp += 30
        doppelt_print(f"{monster_name} wurde besiegt und gab dir etwas {gewinn}!")
        if schaden == 40:
...
def zufalls_event(invetar, spieler_hp, gold, punkte, schaden, angst_punkte):
    ...
    return spieler_hp, gold, punkte, invetar, schaden, angst_punkte
def restart(x, y, spieler_hp, hunger, angst_punkte, invetar, gold, punkte, spiel, ziel):
        entscheidung = input("Willst du einen Bericht von deinem Abenteuer sehen? (Ja/Nein) ")
        ...
        if (weiter_spielen).lower() == "ja":
            spieler_hp = 100
            x, y, spieler_hp, hunger, angst_punkte, invetar, gold, punkte, ziel = start(x, y, spieler_hp, hunger, angst_punkte, invetar, gold, punkte, ziel)
            file_handler.close()
    
            with open('output.txt', 'w', encoding='utf-8') as f:
                f.truncate(0)
        
            file_handler.stream = open('output.txt', 'w', encoding='utf-8') 

        elif (weiter_spielen).lower() == "nein":
            print("Bis dann...")
            spiel = False
        return(x, y, spieler_hp, hunger, angst_punkte, invetar, gold, punkte, spiel, ziel)
...
while spiel:
    while spieler_hp >= 0:
        ...
        if (wahl1).lower() == "links" or (wahl1).lower() == "l":
            x-=10
            #Monsterkampf
            spieler_hp, gold, punkte, invetar, schaden, angst_punkte = zufalls_event(invetar, spieler_hp, gold, punkte, schaden, angst_punkte)
        elif (wahl1).lower() == "rechts" or (wahl1).lower() == "r":
            x+=10
            #Monsterkampf
            spieler_hp, gold, punkte, invetar, schaden, angst_punkte = zufalls_event(invetar, spieler_hp, gold, punkte,schaden, angst_punkte)
        elif (wahl1).lower() == "geradeaus" or (wahl1).lower() == "g":
            y+=10
            spieler_hp, gold, punkte, invetar, schaden, angst_punkte = zufalls_event(invetar, spieler_hp, gold, punkte, schaden, angst_punkte)
        elif (wahl1).lower() == "hinter" or (wahl1).lower() == "h": 
            y-=10
            spieler_hp, gold, punkte, invetar, schaden, angst_punkte = zufalls_event(invetar, spieler_hp, gold, punkte, schaden, angst_punkte)
        else:
        ...
        if x == ziel[0] and y == ziel[1]:
            doppelt_print("Lichtkugel: Oh nein, da wartet der Schattenkönig am Ausgang. Du musst ihn mit deinem Mut besiegen.")
            monster_kampf(invetar, punkte, gold, schaden, angst_punkte,"Der Schattenkönig", spieler_hp, 25)
            if spieler_hp > 0:
                punkte += 1
            break
    ...
    if spieler_hp <= 0:
        ...
        time.sleep(10)
        print("Wir sind da um euch dran zu erinnern wovor ihr Angst habt, aber auch um die Angst zu überwinden. Bis dann...")
        time.sleep(1)
        
        x, y, spieler_hp, hunger, angst_punkte, invetar, gold, punkte, spiel, ziel = restart(x, y, spieler_hp, hunger, angst_punkte, invetar, gold, punkte, spiel, ziel)
    ...
    if angst_punkte == 7:
        doppelt_print("Jetzt bist du einer von uns!!! Du bestehst nur aus Angst. Und das wird so bleiben, bis man dich besiegt. Und jetzt lasse ich dich mit deiner Angst allein. Viel Spaß...")
